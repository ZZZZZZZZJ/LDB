...
function [LowRank,Mask,tau,info,E,C] = ObjDetection_LDB_3D(ImData,CNNData,MaskData,opt)
...
if opt.lambda > 0
    ...
    GCO_SetSmoothCost( hMRF, [0 1;1 0] );
    %AdjMatrix = getAdjFromFlow(sizeTensor,FlowData);
...
%% function to get the adjcent matirx of the graph
function WW = getAdjFromFlow(sizeData,FlowData)
numSites = prod(sizeData);
[H,W,C,T] = size(FlowData);
PosN = zeros(1,numSites);
idx = 1;
for t = 1:T
    for x = 1:H
        for y = 1:W
            dx = round(FlowData(x,y,1,t));
            dy = round(FlowData(x,y,2,t));
            NewX = x+dx;
            NewY = y+dy;
            if (NewX < 1) NewX = 1; end
            if (NewX > H) NewX = H; end
            if (NewY < 1) NewY = 1; end
            if (NewY > W) NewY = W; end    
            Pos       = (t-1)*(H*W) + (x-1)*W + y;
            PosN(idx) = t*(H*W) + (NewX-1)*W + NewY;
            idx = idx + 1;
        end
    end
end
...
%value = ones(1,3*numSites);
value = cat(2,ones(1,2*numSites),0.1*ones(1,numSites));
WW = sparse(id1,id2,value);

WW = WW(1:numSites,1:numSites);
end
