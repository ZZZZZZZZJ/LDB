function [ CNNData ] = Tri_DCNN(ImData,dataname)
...
opts.net_path = '/home/iprai/zzj/CD/lowrank/LRMB/3DCNN/get_train_initial_HSI_V1/net-epoch-25.mat'; 
...
function output_img = NetForward(net,data)
   ...
   for H_k = 1:divide_H
       for W_k = 1:divide_W
            batchCnt = batchCnt + 1;
            x(:,:,:,batchCnt) = data((H_k-1)*patch_size_H+1:H_k*patch_size_H,(W_k-1)*patch_size_W+1:W_k*patch_size_W,:);                
       end
   end
   for Cnt = 1:batchCnt
    ...
    for H_k = 1:divide_H
        for W_k = 1:divide_W
            batchCnt = batchCnt + 1;
            output_img((H_k-1)*patch_size_H+1:H_k*patch_size_H,(W_k-1)*patch_size_W+1:W_k*patch_size_W,:) = output(:,:,:,batchCnt);   
        end
    end
