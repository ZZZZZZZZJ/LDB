...
%  resfilename = strcat('./results/fall/label12/',num2str(i,'%06d'),'.png');
%  label12 = reshape(E.*(label==3)+(E.*(label==2)),imgsize);
%  imwrite((label12+1)/2,resfilename);
%  
%  resfilename = strcat('./results/fall/fig/',num2str(i,'%06d'),'.png');
%  imwrite(I,resfilename);
  S_d = reshape(abs(E).*(label==1),imgsize);
  if (size(S_d,3) == 3)
      S_d = rgb2gray(S_d);
  end
 S = abs(S_d) / max(max(abs(S_d))) > 0.1;
 dirname = strcat(para.outdir,para.dataname);
 if (~exist(dirname,'dir'))
    mkdir(dirname);
 end
 %maskfilename = strcat('./results/',para.dataname,'/mask/bin',num2str(i,'%06d'),'.png');
 maskfilename = strcat(dirname,'/bin',num2str(i,'%06d'),'.png');
 imwrite(S,maskfilename);
end
 end
%% output
end
