function runningtime = RPCA_algorithm(dataname, img_dir,frame_range)
    disp(['perform algorithm RPCA on image dirname:' img_dir, ', frame range is:', num2str(frame_range)]);
    ds_ratio = 1;
    ...
    tic;
    [L_d, S_d, iter]=inexact_alm_rpca(D3,0.006);
    runningtime = toc;
    ...
    resdir = strcat('./results/RPCA/',dataname,'/');
    if (~exist(resdir,'dir'))
        mkdir(resdir);
    end
